...
const Carousel = ({ children, onActive, onHalfActive }: CarouselProps) => {
  ...
  useEffect(() => {
    onHalfActive && onHalfActive(halfActiveIndex);
  }, [halfActiveIndex]);
...
const ItemContainer = ({ children }: ItemContainerProps) => {
  return (
    <div className="flex overflow-auto w-full snap-mandatory snap-x hide-scroll">
      {children}
    </div>
  );
};
...
const Item = ({ children, index }: ItemProps) => {
  const { addItem, deleteItem, setActiveIndex, setHalfActiveIndex } =
    ...
    const fullObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          console.log("fullActiveIndex", index);
  ...
  return (
    <div
      className="inline-flex items-center justify-center w-full flex-none snap-center"
      ref={targetRef}
    >
      {children}
    </div>
...
const Pagination = ({
  ...
  const { activeIndex, length } = useContext(
    CarouselContext
  ) as CarouselContextType;

  const getBulletClass = (index: number) =>
    index === activeIndex ? activeBulletClass : bulletClass;
